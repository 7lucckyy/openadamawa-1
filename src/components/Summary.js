...
const Summary = ({ totalNumberOfProjects, totalContract, highestContract, lowestContract }) => {
    const cards = [
        {title: "Total number of projects", value: totalNumberOfProjects, bgColor: "bg-pacific-blue", icon: FaCog},
        {title: "Total contract sum", value: totalContract, bgColor: "bg-green2", icon: FaMoneyBill},
        {title: "Highest contract amount", value: highestContract, bgColor: "bg-granny-smith-apple", icon: FaBriefcase},
        {title: "Lowest contract amount", value: lowestContract, bgColor: "bg-viridian-green", icon: FaBriefcase}
    ]
  return (
                    ...
                    <Card className={`text-center shadow rounded border-0 ${card.bgColor}`} bg="">
                        <Card.Body>
                            <div className='p-4 px-sm-1 text-white d-flex justify-content-between align-items-center'>
                                ...
                                <h3>
                                    <span className='text-decoration-line-through fw-bold'>N</span>
